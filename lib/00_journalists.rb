...
def getHandleAscend(tab)
    result = tab.min_by{|tab| tab.size}
    return result
end
#Combien y-a-t'il de handle contenant 5 caractères (le @ ne compte pas pour un caractère)
def Hm5character(tab,ordre)
    
    result= tab.select { |word| word.length == 6}
    return result
    
end
tab_ordre = getHandleAscend(handle_tab )
...
#Trie la liste de handle par ordre alphabétique.
    ...
    def alphacase(tab)
        result = tab.sort_by { |tab| tab.downcase }
        return result
    end
...
#Trie la liste de handle par taille des handle (les plus petits en premiers, les plus grands après)
    def lenghttrie(tab)
        result = tab.sort_by{|tab| tab.size}
      return result

    end
#Quelle est la position dans l'array de la personne @epenser
    def findEp(tab)
        result = tab.index {|x| x == "@epenser"};
        return result
    end

    def maxSize(tab)
        result = tab.max_by{|tab| tab.size}
...
#Sors-moi une répartition des handle par taille de ces derniers (nombre de handle avec 1 caractère, nombre de handle avec 2 caractères, nombre de handle avec 3 caractères, etc)
    def repartition(tab,max)
        result = nil;

        max.times do |i|
           list = tab.select{|tab| tab.size ==i}
           if list != []
            print "il y a #{list.length()} journaliste qui on #{i} de charactere , il sont :\n#{list}\s"
         
           end
           
        end
    end
getHandleAscend(handle_tab)
lenghttrie(handle_tab)
handle_tab[findEp(handle_tab)]
repartition(handle_tab,maxSize(handle_tab))
